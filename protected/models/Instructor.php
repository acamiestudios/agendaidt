...
class Instructor extends CActiveRecord
{
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function getNombreCompletoTodos(){
            $model = self::model()->findAll();
            if($model !== null ){
                $idts = array();
                foreach ($model as $idt) {
                    $list[$idt->idInstructor] = $idt->primer_nombre . ' ' . $idt->primer_apellido;
                }
            }
            return $list;
        }
        
        public function getNombreCompletoId($id){
