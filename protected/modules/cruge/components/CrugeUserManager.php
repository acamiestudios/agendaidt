...
class CrugeUserManager
{
    ...
    */
	 ...
	 */
	public function searchUsersByAuthItem($authItemName, $pageSize=20, $defaultOrder=null, $_model=null){
		if($_model == null){
        ...
        public function searchUsersByAuthItemArr($authItemName, $_model=null){
            if($_model == null){
                $model = $this->getSearchUserModel();
            }else
                $model = $_model;
            return $model->searchByAuthItemArr($authItemName);
	}
    ...
    */
    public function getLabelField(ICrugeField $field)
    {
        ...
        return "<label class='col-md-4 control-label {$lbr}'>{$text} {$ast}</label>";
    }
    ...
    */
    ...
    public function createNewUser($mapped_values,$role_name='default', $bool_send_email=true)
    {
        ...
        if( isset($mapped_values['state'])){
            if( $mapped_values['state'] == 1 ){
