...
class CrugeStoredUser extends CActiveRecord implements ICrugeStoredUser
{
    ...
    */
	...
	public function searchByAuthItem($authItemName, $pageSize=20, $defaultOrder=null){
        ...
        /*
         * select * from cruge_user t
left join cruge_authassignment ASG on ASG.userid = t.iduser
left join cruge_fieldvalue tbFv on tbFv.iduser = t.iduser
inner join cruge_field tbF on tbF.idfield = tbFv.idfield
where ASG.itemname='Instructor';
         */
        public function searchByAuthItemArr($authItemName){
            $sql=Yii::app()->db->createCommand("select t.iduser,CONCAT_WS(' - ',t.username, tbFv.value) idt from cruge_user t
                    left join cruge_authassignment ASG on ASG.userid = t.iduser
                    inner join cruge_fieldvalue tbFv on tbFv.iduser = t.iduser
                    inner join cruge_field tbF on tbF.idfield = tbFv.idfield
                    where ASG.itemname='Instructor' and tbF.fieldname='apellidos'");/*
                    union
                    (select t.iduser,tbFv.value from cruge_user t
                    left join cruge_authassignment ASG on ASG.userid = t.iduser
                    left join cruge_fieldvalue tbFv on tbFv.iduser = t.iduser
                    inner join cruge_field tbF on tbF.idfield = tbFv.idfield
                    where ASG.itemname='Instructor' and tbF.fieldname='apellidos') ");*/
            return $sql->queryAll();
            /*
            $sql->selectDistinct("t.iduser,t.username,tbFv.value");
            $sql->from("cruge_user t");
            $sql->join("cruge_authassignment ASG","ASG.userid=t.iduser");
            $sql->join("cruge_fieldvalue tbFv","tbFv.iduser= t.iduser");
            $sql->join("cruge_field tbF","tbF.idfield = tbFv.idfield");
            $sql->where("ASG.itemname='Instructor' and tbF.fieldname='apellidos'");
            $sql2=Yii::app()->db->createCommand();
            $sql2->selectDistinct("t.iduser,t.username,tbFv.value");
            $sql2->from("cruge_user t");
            $sql2->join("cruge_authassignment ASG","ASG.userid=t.iduser");
            $sql2->join("cruge_fieldvalue tbFv","tbFv.iduser= t.iduser");
            $sql2->join("cruge_field tbF","tbF.idfield = tbFv.idfield");
            $sql2->where("ASG.itemname='Instructor' and tbF.fieldname='apellidos'");
            $sql->union($sql2);
            return $sql->queryAll();*/
            /*$criteria = new CDbCriteria;
            $criteria->distinct = true;
            ...
            $criteria->join = "left join ".$table_assign." ASG "
                    ."on ASG.userid = t.iduser";
            $criteria->compare("ASG.itemname",$authItemName);
            $fieldMan = new CrugeFieldValue();
            $tabla_field = $fieldMan->tableName();
            $criteria->join .=" left join " . $tabla_field. " tbFv on tbFv.iduser = t.iduser";
            $criteria->join .=" inner join cruge_field tbF on tbF.idfield = tbFv.idfield";
